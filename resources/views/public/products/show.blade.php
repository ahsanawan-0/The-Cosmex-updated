            ...
            <div class="grid gap-8 lg:grid-cols-2 lg:gap-16">
                ...
                <div class="flex flex-col-reverse gap-4 sm:flex-row">
                    ...
                    @if(count($allImages) > 1)
                        <div
                            class="flex gap-2 sm:flex-col sm:gap-3 sm:w-[72px] shrink-0 overflow-x-auto sm:overflow-y-auto sm:max-h-[560px] hide-scrollbar">
                ...
                <div class="flex flex-col rounded-[28px] bg-white p-5 shadow-card sm:p-7 lg:p-8">
                    ...
                    {{-- Name --}}
                    <h1
                        class="mt-4 font-heading text-2xl font-bold leading-snug text-text-primary sm:text-3xl lg:text-[36px]">
                        {{ $product->name }}</h1>
                    ...
                    <div class="mt-3 flex items-center gap-2">
                        <div class="flex gap-0.5">
                            @for($i = 1; $i <= 5; $i++)
                                <i
                                    class="fa-{{ $i <= $avgRating ? 'solid' : 'regular' }} fa-star text-[12px] {{ $i <= $avgRating ? 'text-amber-400' : 'text-zinc-300' }}"></i>
                            @endfor
                        </div>
                        <span
                            class="text-xs text-zinc-400">{{ $reviews->count() > 0 ? $reviews->count() . ' review' . ($reviews->count() > 1 ? 's' : '') : 'No reviews yet' }}</span>
                    </div>

                    {{-- Price --}}
                    <div class="mt-5 flex flex-wrap items-baseline gap-3">
                        @if($product->is_on_sale)
                            <span class="font-heading text-3xl font-bold text-primary">PKR
                    ...
                    <div class="mt-5 flex items-center gap-3">
                        ...
                        <button
                            onclick="navigator.clipboard.writeText('{{ $canonical }}').then(()=>{this.title='Copied!';setTimeout(()=>this.title='Copy Link',2000)})"
                            class="flex h-10 w-10 items-center justify-center rounded-full border border-border text-text-secondary transition hover:border-primary hover:text-primary"
                            title="Copy Link">
                    ...
                    <div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                        <div
                            class="flex flex-col items-center gap-1.5 rounded-2xl border border-border bg-bg-light p-3 text-center">
            ...
            <div id="pdp-panel-delivery" class="pdp-panel hidden pt-8">
                <ul class="space-y-4 text-sm text-text-secondary">
                    <li class="flex items-start gap-3"><i
                            class="fa-solid fa-truck mt-0.5 text-zinc-400"></i><span><strong>Nationwide Delivery:</strong>
                            3–7 business days across Pakistan.</span></li>
                    <li class="flex items-start gap-3"><i
                            class="fa-solid fa-bolt mt-0.5 text-zinc-400"></i><span><strong>Lahore:</strong> Same day or
                            next day delivery available.</span></li>
                    <li class="flex items-start gap-3"><i
                            class="fa-solid fa-money-bill mt-0.5 text-zinc-400"></i><span><strong>Payment:</strong> Cash on
                            Delivery (COD) available nationwide.</span></li>
                    <li class="flex items-start gap-3"><i
                            class="fa-solid fa-box mt-0.5 text-zinc-400"></i><span><strong>Packaging:</strong> Products are
                            carefully packed to ensure safe delivery.</span></li>
                    <li class="flex items-start gap-3"><i
                            class="fa-solid fa-rotate-left mt-0.5 text-zinc-400"></i><span><strong>Returns:</strong> Contact
                            us within 3 days if there's an issue.</span></li>
                </ul>
            </div>
            ...
            <div id="pdp-panel-reviews" class="pdp-panel hidden pt-8">
                    ...
                    <div class="flex flex-col sm:flex-row gap-8 mb-10 p-6 bg-zinc-50 rounded-2xl border border-zinc-100">
                        <div class="text-center shrink-0">
                            ...
                            <div class="flex justify-center gap-0.5 mt-2">
                                @for($i = 1; $i <= 5; $i++)
                                    <i
                                        class="fa-{{ $i <= $avgRating ? 'solid' : 'regular' }} fa-star text-sm {{ $i <= $avgRating ? 'text-amber-400' : 'text-zinc-300' }}"></i>
                                @endfor
                            </div>
                            <p class="text-xs text-zinc-400 mt-1">{{ $reviews->count() }}
                                review{{ $reviews->count() > 1 ? 's' : '' }}</p>
                        </div>
                        <div class="flex-1 space-y-2">
                            @for($star = 5; $star >= 1; $star--)
                                @php $count = $reviews->where('rating', $star)->count();
                                $pct = $reviews->count() > 0 ? ($count / $reviews->count()) * 100 : 0; @endphp
                                <div class="flex items-center gap-3 text-xs">
                                    ...
                                    <div class="flex-1 h-2 bg-zinc-200 rounded-full overflow-hidden">
                                        <div class="h-full bg-amber-400 rounded-full transition-all" style="width: {{ $pct }}%">
                                        </div>
                                    </div>
                                ...
                                <div class="flex items-start justify-between gap-4">
                                    <div class="flex items-center gap-3">
                                        <div class="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                                            <span
                                                class="text-sm font-bold text-primary uppercase">{{ substr($review->reviewer_name, 0, 1) }}</span>
                                        </div>
                                        <div>
                                    ...
                                    <div class="flex gap-0.5 shrink-0">
                                        @for($i = 1; $i <= 5; $i++)
                                            <i
                                                class="fa-{{ $i <= $review->rating ? 'solid' : 'regular' }} fa-star text-[11px] {{ $i <= $review->rating ? 'text-amber-400' : 'text-zinc-300' }}"></i>
                                        @endfor
                                    </div>
                                </div>
                ...
                <div class="rounded-2xl border border-border bg-bg-light p-6 lg:p-8">
                    ...
                    <form action="{{ route('reviews.store', $product->slug) }}" method="POST" class="space-y-5">
                        ...
                        {{-- Star Rating Picker --}}
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Your
                                Rating</label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label class="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Your
                                    Name *</label>
                                <input type="text" name="reviewer_name" required value="{{ old('reviewer_name') }}"
                            ...
                            <div>
                                <label class="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Email
                                    (optional)</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Your Review
                                *</label>
                            <textarea name="body" required rows="4" placeholder="Share your experience with this product..."
